...
public class CacheableWebViewTest {
    ...
    public void setUp() {
        ...
        shadowConnectivityManager = shadowOf((ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE));
        webView = new CacheableWebView(context);
    }

    @Test
    public void testDefaultFileAccess() {
        // Currently true by default (before fix), we expect this to fail after we add the assertion for false.
        // But for TDD, let's assert what we WANT (false).
        // If the code is not fixed yet, this test should fail.
        assertFalse("File access should be disabled by default", webView.getSettings().getAllowFileAccess());
    }
    ...
    public void testReloadUrlNormalUrlDisablesFileAccess() {
        ...
        assertFalse("File access should be disabled for http URLs", webView.getSettings().getAllowFileAccess());
    }
    ...
    public void testReloadUrlFileSchemeDisablesFileAccess() {
        ...
        assertFalse("File access should be disabled for file:/// scheme", webView.getSettings().getAllowFileAccess());
    }
    ...
    public void testReloadUrlBlankSchemeDisablesFileAccess() {
        ...
        assertFalse("File access should be disabled for about:blank", webView.getSettings().getAllowFileAccess());
    }
    ...
    public void testReloadUrlCachedFileEnablesFileAccess() throws Exception {
        ...
        String cacheFileName = context.getCacheDir().getAbsolutePath() + File.separator + "webarchive-" + name + ".mht";
        ...
        // Since WebView wraps the client, we might need to access the wrapper or just trigger it on the view if possible.
        ...
        // 4. Assert
        assertTrue("File access should be enabled for cached file", webView.getSettings().getAllowFileAccess());
