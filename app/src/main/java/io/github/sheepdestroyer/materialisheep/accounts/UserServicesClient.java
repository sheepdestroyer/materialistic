...
public class UserServicesClient implements UserServices {
    ...
    private static final Pattern REGEX_INPUT = Pattern.compile("<\\s*input[^>]*>");
    private static final Pattern REGEX_VALUE = Pattern.compile("value[^\"]*\"([^\"]*)\"");
    private static final Pattern REGEX_CREATE_ERROR_BODY = Pattern.compile("<body>([^<]*)");
    ...
    private String getInputValue(String html, String name) {
        // extract <input ... >
        Matcher matcherInput = REGEX_INPUT.matcher(html);
        while (matcherInput.find()) {
            String input = matcherInput.group();
            if (input.contains(name)) {
                // extract value="..."
                Matcher matcher = REGEX_VALUE.matcher(input);
    ...
    private String parseLoginError(Response response) {
        try {
            Matcher matcher = REGEX_CREATE_ERROR_BODY.matcher(response.body().string());
