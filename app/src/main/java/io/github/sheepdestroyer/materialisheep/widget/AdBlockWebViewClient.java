  ...
  private boolean isBlockedFile(WebView view, String url) {
    ...
    try {
      ...
      if (lowerUrl.startsWith("file:///android_asset/")) {
        ...
        // Security: Validate canonical path matches expected cache directory to prevent arbitrary
        // local file reads
        if (cacheDir == null
            || !canonicalPath.startsWith(cacheDir.getCanonicalPath() + java.io.File.separator)) {
          return true;
        }
      }
      return false;
