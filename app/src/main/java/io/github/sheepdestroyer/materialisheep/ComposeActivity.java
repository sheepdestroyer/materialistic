...
public class ComposeActivity extends ThemedActivity {
    ...
    private String createQuote() {
        if (mQuoteText == null) {
            String text = AppUtils.fromHtml(mParentText).toString().trim();
            mQuoteText = String.format(FORMAT_QUOTE, PATTERN_PARAGRAPH_BREAK.matcher(text).replaceAll(PARAGRAPH_QUOTE));
        }
        return mQuoteText;
    }
